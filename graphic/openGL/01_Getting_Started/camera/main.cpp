...
int main()
{
    ...
    string s0(cwd);
//    cout << (s0+"/shader.vs") << endl;
    Shader ourShader((s0+"/shader.vs").c_str(), (s0+"/shader.fs").c_str());
    ...
    unsigned char *data = stbi_load((s0+"/container.jpg").c_str(), &width, &height, &nrChannels, 0);
    if (data) {
    ...
    data = stbi_load((s0+"/awesomeface.png").c_str(), &width, &height, &nrChannels, 0);
    if (data) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        for (unsigned int i = 0; i< 10; i++) {
            ...
            glm::mat4 view = ourCamera.GetViewMatrix(); // glm::lookAt(cameraPos, cameraPos + cameraFront, cameraUp);
